...
export default class AccountsController extends Controller {
                ...
                if (user != null) {
                    if (user.Password == loginInfo.Password) {
                        ...
                        if (user.Authorizations.readAccess == -1 && user.Authorizations.writeAccess == -1) {
                            this.HttpContext.response.blocked('Blocked');
                        } else {
        ...
        if (AccessControl.writeGranted(this.HttpContext.authorizations, AccessControl.user())) {
            if (this.repository != null) {
                ...
                if (foundedUser != null) {
                    ...
                    if(user.Avatar== '' ){
                        this.repository.update(user.Id, user,false);
                    }else{
    ...
    remove(id) { // warning! this is not an API endpoint 
        ...
        if (AccessControl.writeGrantedAdminOrOwner(this.HttpContext, this.requiredAuthorizations, id)) {
            ...
            let _PostController = new PostsController(this.HttpContext);
            let _LikeController = new LikesController(this.HttpContext);
            // remove likes of user
            _LikeController.repository.keepByFilter(function (obj) { return obj.User != id });
            // get posts of user
            ...
            _PostController.repository.keepByFilter(function (obj) { return obj.OwnerId != id });
    ...
    checktoken() {
        if (this.HttpContext.path.params.token != undefined) {
            let acces = this.HttpContext.path.params.token;
            let token = TokenManager.findAccesToken(acces);

            if (token != null) {
                this.HttpContext.response.JSON(token);
            } else {
                this.HttpContext.response.notFound();
            }
    ...
    getusers() {
        if (this.HttpContext.path.params['ids[]'] != undefined) {
            ...
            if (!Array.isArray(ids)) {
                let user = this.repository.get(ids);
                if (user == null) {
                    notFound = true;
                } else {
                    let baseInfoUser = {};
                    baseInfoUser.Avatar = user.Avatar;
                    baseInfoUser.Name = user.Name;
                    users.push(baseInfoUser);
                }
            }else{
                for (let i = 0; i < ids.length; i++) {

                    let user = this.repository.get(ids[i]);
                    if (user == null) {
                        notFound = true;
                    } else {
                        let baseInfoUser = {};
